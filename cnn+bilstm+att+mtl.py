...
print(train_X.shape,test_X.shape,train_f.shape,test_f.shape)
...
m.compile(
    ...
    loss_weights={'outf': 1.0, 'outs':1.0},
    metrics=['mape']
    )#
history=m.fit(train_X, [train_f,train_s], epochs=20, batch_size=256, validation_split=0.02967,verbose=2,shuffle=False)
#batch size一般为2的n次方
plt.figure()
...
plt.title('loss')
plt.figure()
plt.plot(history.history['val_outf_mean_absolute_percentage_error'],'g',label='val_outf_mape')
plt.plot(history.history['val_outs_mean_absolute_percentage_error'],'y',label='val_outs_mape')
plt.legend(loc='upper left')
plt.title('error')

pred_y=m.predict(test_X,batch_size=64)
